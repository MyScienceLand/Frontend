  ...
  return (
    <div className="flex flex-col items-center h-full mt-28  md:flex-row  md:gap-4 justify-center ">
      <div className="text-center md:mt-8 md:flex-1 md:pl-16">
        <p className="text-[var(--text-color)] text-lg mb-2">
          WE CAN’T FOUND THIS PAGE
        </p>
        <h1 className="text-6xl font-bold text-[var(--text-color)] mb-2">
          404
        </h1>
        <p className="text-xl text-[var(--text-color)] mb-4">PAGE LOST</p>
        <button
          onClick={() => (window.location.href = "/")}
          className="mt-6 px-4 py-2 bg-[var(--secondary-color)] text-[var(--primary-color)] rounded-lg hover:bg-purple-700"
        >
          Go Back Home
        </button>
      </div>
